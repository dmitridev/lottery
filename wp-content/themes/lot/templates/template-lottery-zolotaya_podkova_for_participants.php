...
<?php
/*
Template Name: Лотерея zolotaya_podkova_for_participants
*/
//get_header();

global $wpdb;

$res = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='zolotaya_podkova_for_participants'", ARRAY_A);
?>
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='zolotaya_podkova_for_participants'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_zolotaya_podkova_for_participants`", ARRAY_A);
    ?>
    ...
    <td>
        ...
        async function update_data() {
            ...
            formData.append('action', 'insert_zolotaya_podkova_for_participants');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_zolotaya_podkova_for_participants');
