...
<?php
/*
Template Name: Лотерея top_3_for_gamers
*/
//get_header();
...
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='top_3_for_gamers'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_top_3_for_gamers`", ARRAY_A);
    ?>
	...
	const NUMBER3 = !!tr.querySelector('[data-name="NUMBER3"]').value ? tr.querySelector('[data-name="NUMBER3"]').value:0;
            ...
            formData.append('action', 'insert_top_3_for_gamers');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_top_3_for_gamers');
