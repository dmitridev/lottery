...
<?php
/*
Template Name: Лотерея 12_24_for_participants
*/
//get_header();
...
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='12_24_for_participants'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_12_24_for_participants`", ARRAY_A);
    ?>
	...
	const NUMBER12 = !!tr.querySelector('[data-name="NUMBER12"]').value ? tr.querySelector('[data-name="NUMBER12"]').value:0;
            ...
            formData.append('action', 'insert_12_24_for_participants');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_12_24_for_participants');
