...
<?php
/*
Template Name: Лотерея lavina_prizov_for_participants
*/
//get_header();

global $wpdb;

$res1 = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='lavina_prizov_for_participants' and `VALUE_ID` LIKE \"FIELD_1%\"", ARRAY_A);
$res2 = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='lavina_prizov_for_participants' and `VALUE_ID` LIKE \"FIELD_2%\"",ARRAY_A);
?>
...
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='lavina_prizov_for_participants'");
    
    $table = $wpdb->get_results("SELECT * from `wp_lottery_lavina_prizov_for_participants`", ARRAY_A);
    ...
    <script>
        ...
        async function add_new() {
            ...
            <td style="padding-right:10px;">Поле 1:</td>
    ...
    <td style="padding-right:10px;">Поле 2:</td>
    ...
    <td><input data-name="NUMBER8" value=""></td>
        ...
        async function update_data() {
            ...
            Array.from(document.querySelectorAll('.table-row')).forEach((tr, index) => {
                ...
                const CURRENT = index;
                array.push({
                    numbers: [NUMBER1, NUMBER2, NUMBER3, NUMBER4,NUMBER5,NUMBER6,NUMBER7,NUMBER8],
                    number: CURRENT
                });
            });
            ...
            formData.append('action', 'insert_lavina_prizov_for_participants');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_lavina_prizov_for_participants');
    ...
    <section class="lottery" style="margin-top:100px">
        <div class="container">
            <table class="lottery-table" cellspacing="0" cellpadding="0">
                ...
                <tbody>
                    <?print('<tr><td colspan="6">Поле 1:<td></tr>');?>
                    <?php foreach ($res1 as $index => $row) { ?>
                        <?php
                        ...
                        <tr id="<?= $row['VALUE_ID'] ?>">
                            ...
                            <th class="<?php if (str_starts_with($row['VALUE_ID'], 'FIELD_'))
                                echo "blue-header";
                            else
                                echo "green-header"; ?>
                                    "><?= $row['VALUE_CONDITION'] ?>
                    ...
                    <?php } ?>
                    <? print('<tr><td colspan="6">Поле 2:</td></tr>');?>
                    <?php foreach ($res2 as $index => $row) { ?>
                        <?php
                        $diff_yes = abs($row['VALUE_YES'] - $row['VALUE_YES_NOW']);
                        $diff_no = abs($row['VALUE_NO'] - $row['VALUE_NO_NOW']);
                        // разница между значением "сейчас" и "макс" от 0 до 6 подсвечивается разными цветами.
