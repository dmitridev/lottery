...
<?php
/*
Template Name: Лотерея 3_3_for_participants
*/
//get_header();

global $wpdb;

$res = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='3_3_for_participants'", ARRAY_A);
?>
...
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='3_3_for_participants'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_3_3_for_participants`", ARRAY_A);
    ?>
	...
	const NUMBER15 = !!tr.querySelector('[data-name="NUMBER15"]').value ? tr.querySelector('[data-name="NUMBER15"]').value:0;
                const CURRENT = index;
                array.push({
                    numbers: [NUMBER1,NUMBER2,NUMBER3,NUMBER4,NUMBER5,NUMBER6,NUMBER7,NUMBER8,NUMBER9,NUMBER10,NUMBER11,NUMBER12,NUMBER13,NUMBER14,NUMBER15],
                    number: CURRENT
                });
            });
            ...
            formData.append('action', 'insert_3_3_for_participants');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_3_3_for_participants');
