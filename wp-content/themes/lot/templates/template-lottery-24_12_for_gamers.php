...
<?php
/*
Template Name: Лотерея 24_12_for_gamers
*/
//get_header();

global $wpdb;

$res = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='24_12_for_gamers'", ARRAY_A);
?>
<style>
...
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='24_12_for_gamers'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_24_12_for_gamers`", ARRAY_A);
    ?>
	...
	const NUMBER12 = !!tr.querySelector('[data-name="NUMBER12"]').value ? tr.querySelector('[data-name="NUMBER12"]').value:0;
                const CURRENT = index;
                array.push({
                    numbers: [NUMBER1,NUMBER2,NUMBER3,NUMBER4,NUMBER5,NUMBER6,NUMBER7,NUMBER8,NUMBER9,NUMBER10,NUMBER11,NUMBER12],
                    number: CURRENT
                });
            });
            ...
            formData.append('action', 'insert_24_12_for_gamers');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_24_12_for_gamers');
