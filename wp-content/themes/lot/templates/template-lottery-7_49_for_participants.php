...
<?php
/*
Template Name: Лотерея 7_49_for_participants
*/
//get_header();

global $wpdb;

$res = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='7_49_for_participants'", ARRAY_A);
?>
<style>
...
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='7_49_for_participants'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_7_49_for_participants`", ARRAY_A);
    ?>
	...
	const NUMBER7 = !!tr.querySelector('[data-name="NUMBER7"]').value ? tr.querySelector('[data-name="NUMBER7"]').value:0;
            ...
            formData.append('action', 'insert_7_49_for_participants');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_7_49_for_participants');
