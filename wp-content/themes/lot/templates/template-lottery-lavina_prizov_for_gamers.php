...
<?php
/*
Template Name: Лотерея lavina_prizov_for_gamers
*/
//get_header();

global $wpdb;

$res = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='lavina_prizov_for_gamers'", ARRAY_A);
?>
<style>
...
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='lavina_prizov_for_gamers'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_lavina_prizov_for_gamers`", ARRAY_A);
    ?>
	...
	const NUMBER4 = tr.querySelector('[data-name="NUMBER4"]').value;
	const NUMBER5 = tr.querySelector('[data-name="NUMBER5"]').value;
	const NUMBER6 = tr.querySelector('[data-name="NUMBER6"]').value;
	const NUMBER7 = tr.querySelector('[data-name="NUMBER7"]').value;
	const NUMBER8 = tr.querySelector('[data-name="NUMBER8"]').value;
                const CURRENT = index;
                array.push({
            ...
            formData.append('action', 'insert_lavina_prizov_for_gamers');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_lavina_prizov_for_gamers');
