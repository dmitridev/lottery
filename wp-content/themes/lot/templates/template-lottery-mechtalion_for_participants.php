...
<?php
/*
Template Name: Лотерея mechtalion_for_participants
*/
//get_header();
...
$res = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='mechtalion_for_participants'", ARRAY_A);
?>
...
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='mechtalion_for_participants'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_mechtalion_for_participants`", ARRAY_A);
    ?>
    ...
    <td><input data-name="NUMBER36" value=""></td>
    <td><input data-name="NUMBER37" value=""></td>
        ...
        async function update_data() {
            ...
            Array.from(document.querySelectorAll('.table-row')).forEach((tr, index) => {
                ...
                const NUMBER36 = !!tr.querySelector('[data-name="NUMBER36"]').value ? tr.querySelector('[data-name="NUMBER36"]').value : 0;
                const NUMBER37 = !!tr.querySelector('[data-name="NUMBER37"]').value ? tr.querySelector('[data-name="NUMBER37"]').value : 0;
                const CURRENT = index;
                array.push({
                    numbers: [NUMBER1, NUMBER2, NUMBER3, NUMBER4, NUMBER5, NUMBER6, NUMBER7, NUMBER8, NUMBER9, NUMBER10, NUMBER11, NUMBER12, NUMBER13, NUMBER14, NUMBER15, NUMBER16, NUMBER17, NUMBER18, NUMBER19, NUMBER20, NUMBER21, NUMBER22, NUMBER23, NUMBER24, NUMBER25, NUMBER26, NUMBER27, NUMBER28, NUMBER29, NUMBER30, NUMBER31, NUMBER32, NUMBER33, NUMBER34, NUMBER35, NUMBER36, NUMBER37],
                    number: CURRENT
                });
            });
            ...
            formData.append('action', 'insert_mechtalion_for_participants');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_mechtalion_for_participants');
                ...
                <tbody>
                    <?php foreach ($table as $row) { ?>
                        <tr class="table-row">
                            ...
                            <td><input data-name="NUMBER36" value="<?= $row['NUMBER36'] ?>"></td>
