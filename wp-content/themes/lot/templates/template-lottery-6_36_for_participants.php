...
<?php
/*
Template Name: Лотерея 6_36_for_participants
*/
//get_header();

global $wpdb;

$res = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='6_36_for_participants'", ARRAY_A);
?>
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='6_36_for_participants'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_6_36_for_participants`", ARRAY_A);
    ?>
	...
	const NUMBER6 = !!tr.querySelector('[data-name="NUMBER6"]').value ? tr.querySelector('[data-name="NUMBER6"]').value:0;
            ...
            formData.append('action', 'insert_6_36_for_participants');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_6_36_for_participants');
