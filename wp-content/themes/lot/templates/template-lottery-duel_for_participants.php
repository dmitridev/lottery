...
<?php
/*
Template Name: Лотерея duel_for_participants
*/
//get_header();

global $wpdb;

$res1 = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='duel_for_participants' and `VALUE_ID` LIKE \"FIELD_1%\"", ARRAY_A);
$res2 = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='duel_for_participants' and `VALUE_ID` LIKE \"FIELD_2%\" ", ARRAY_A);
?>
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='duel_for_participants'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_duel_for_participants`", ARRAY_A);
    ?>
    ...
    <script>
        ...
        async function update_data() {
            ...
            formData.append('action', 'insert_duel_for_participants');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_duel_for_participants');
