...
<?php
/*
Template Name: Лотерея velikolepnaya_8_for_gamers
*/
//get_header();

global $wpdb;

$res = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='velikolepnaya_8_for_gamers'", ARRAY_A);
?>
<style>
...
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='velikolepnaya_8_for_gamers'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_velikolepnaya_8_for_gamers`", ARRAY_A);
    ?>
	...
	const NUMBER9 = tr.querySelector('[data-name="NUMBER9"]').value;
                const CURRENT = index;
                array.push({
            ...
            formData.append('action', 'insert_velikolepnaya_8_for_gamers');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_velikolepnaya_8_for_gamers');
