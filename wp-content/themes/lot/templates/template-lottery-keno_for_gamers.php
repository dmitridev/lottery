...
<?php
/*
Template Name: Лотерея keno_for_gamers
*/
//get_header();

global $wpdb;

$res = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='keno_for_gamers'", ARRAY_A);
?>
...
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='keno_for_gamers'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_keno_for_gamers`", ARRAY_A);
    ?>
	...
	const NUMBER22 = !!tr.querySelector('[data-name="NUMBER22"]').value ? tr.querySelector('[data-name="NUMBER22"]').value:0;
            ...
            formData.append('action', 'insert_keno_for_gamers');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_keno_for_gamers');
