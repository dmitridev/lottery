...
<?php
/*
Template Name: Лотерея 6_45_for_gamers
*/
//get_header();

global $wpdb;

$res = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='6_45_for_gamers'", ARRAY_A);
?>
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='6_45_for_gamers'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_6_45_for_gamers`", ARRAY_A);
    ?>
	...
	const NUMBER6 = !!tr.querySelector('[data-name="NUMBER6"]').value ? tr.querySelector('[data-name="NUMBER6"]').value:0;
            ...
            formData.append('action', 'insert_6_45_for_gamers');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_6_45_for_gamers');
