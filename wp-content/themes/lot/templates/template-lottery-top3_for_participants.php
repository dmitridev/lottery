...
<?php
/*
Template Name: Лотерея top3_for_participants
*/
//get_header();

global $wpdb;

$res = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='top3_for_participants'", ARRAY_A);
?>
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='top3_for_participants'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_top3_for_participants`", ARRAY_A);
    ?>
	...
	const NUMBER3 = !!tr.querySelector('[data-name="NUMBER3"]').value ? tr.querySelector('[data-name="NUMBER3"]').value:0;
            ...
            formData.append('action', 'insert_top3_for_participants');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_top3_for_participants');
