...
<?php
/*
Template Name: Лотерея rus_loto_for_participants
*/
//get_header();

global $wpdb;

$res = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='rus_loto_for_participants'", ARRAY_A);
?>
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='rus_loto_for_participants'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_rus_loto_for_participants`", ARRAY_A);
    ?>
    ...
    <script>
        ...
        async function update_data() {
            ...
            formData.append('action', 'insert_rus_loto_for_participants');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_rus_loto_for_participants');
                ...
                <tbody>
                    <?php foreach ($table as $row) { ?>
                        <tr class="table-row">
                            ...
                            <td>
                                <button class="delete_data" onclick="delete_data(this)"
                                    style="margin-right:10px;">x</button>
