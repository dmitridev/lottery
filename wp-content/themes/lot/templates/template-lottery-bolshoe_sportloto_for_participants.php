...
<?php
/*
Template Name: Лотерея bolshoe_sportloto_for_participants
*/
//get_header();

global $wpdb;

$res1 = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='bolshoe_sportloto_for_participants' and `VALUE_ID` LIKE \"NUM%\"", ARRAY_A);
$res2 = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='bolshoe_sportloto_for_participants' and `VALUE_ID` LIKE \"COLUMN%\"", ARRAY_A);
?>
...
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='bolshoe_sportloto_for_participants'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_bolshoe_sportloto_for_participants`", ARRAY_A);
    ?>
    ...
    <script>
        ...
        async function add_new() {
            ...
            tr.innerHTML = `<td data-name="ID"></td>
                        ...
                        <td style="padding-left:10px;padding-right:10px;">Столбец:</td>
        ...
        async function update_data() {
            ...
            formData.append('action', 'insert_bolshoe_sportloto_for_participants');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_bolshoe_sportloto_for_participants');
                ...
                <tbody>
                    <?php foreach ($table as $row) { ?>
                        <tr class="table-row">
                            ...
                            <td style="padding-left:10px;padding-right:10px;">Поле 2:</td>
