...
<?php
/*
Template Name: Лотерея loto_express_for_participants
*/
//get_header();

global $wpdb;

$res = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='loto_express_for_participants'", ARRAY_A);
?>
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='loto_express_for_participants'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_loto_express_for_participants`", ARRAY_A);
    ?>
	...
	const NUMBER4 = !!tr.querySelector('[data-name="NUMBER4"]').value ? tr.querySelector('[data-name="NUMBER4"]').value:0;
            ...
            formData.append('action', 'insert_loto_express_for_participants');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_loto_express_for_participants');
