...
<?php
/*
Template Name: Лотерея 4_20_for_gamers
*/
//get_header();

global $wpdb;

$res = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='4_20_for_gamers'", ARRAY_A);
?>
...
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='4_20_for_gamers'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_4_20_for_gamers`", ARRAY_A);
    ?>
    ...
    <script>
        ...
        async function update_data() {
            ...
            formData.append('action', 'insert_4_20_for_gamers');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_4_20_for_gamers');
