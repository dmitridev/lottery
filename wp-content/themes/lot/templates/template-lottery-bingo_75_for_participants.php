...
<?php
/*
Template Name: Лотерея bingo_75_for_participants
*/
//get_header();

global $wpdb;

$res = $wpdb->get_results("SELECT * FROM `wp_lottery_results` where `LOTO_TYPE`='bingo_75_for_participants'", ARRAY_A);
?>
<main class="for-participants пав">
    ...
    <?php
    $update = $wpdb->get_var("SELECT LAST_UPDATE from `wp_lottery_update` where LOTTERY_NAME='bingo_75_for_participants'");
    $table = $wpdb->get_results("SELECT * from `wp_lottery_bingo_75_for_participants`", ARRAY_A);
    ?>
	...
	const NUMBER3 = !!tr.querySelector('[data-name="NUMBER3"]').value ? tr.querySelector('[data-name="NUMBER3"]').value:0;
            ...
            formData.append('action', 'insert_bingo_75_for_participants');
        ...
        async function update_table($this) {
            ...
            $this.disabled = true;
            const formData = new FormData();
            formData.append("action", 'get_bingo_75_for_participants');
